...
import { 
  ...
  DatePicker, Upload, Select, InputNumber, Image, Tag, Spin 
} from 'antd';
import { 
...
import { 
  getAllVouchersApi, deleteVoucherApi, createVoucher, 
  getVoucherCategory, getVoucherPlatform, uploadApi 
} from '../../../utils/api';
...
const VoucherManagement = () => {
  ...
  const fetchVouchers = async (page = 1, pageSize = 10, search = '') => {
    ...
    try {
      ...
      const processedVouchers = (data?.vouchers || data || []).map(voucher => ({
        key: voucher._id,
        id: voucher._id,
        code: voucher.code,
  ...
  const showEditModal = (voucher) => {
    ...
    form.setFieldsValue({
      ...
      discountValue: voucher.discountValue,
      price: voucher.price,
      expirationDate: voucher.expirationDate ? dayjs(voucher.expirationDate) : undefined,
    });
    setModalVisible(true);
  };
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      message.error('Không thể tải hình ảnh lên: ' + (error.response?.data?.message || error.message));
    } finally {
      setUploading(false);
    }
