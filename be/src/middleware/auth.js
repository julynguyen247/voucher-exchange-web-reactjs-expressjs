...
const auth = (req, res, next) => {
  ...
  if (Date.now() - authMetrics.lastReported > 3600000) {
    console.log(
      `Auth Metrics: ${authMetrics.total} total, ${
        authMetrics.cacheHits
      } cache hits (${Math.round(
        (authMetrics.cacheHits / authMetrics.total) * 100
      )}%), ${authMetrics.cacheMisses} cache misses (${Math.round(
        (authMetrics.cacheMisses / authMetrics.total) * 100
      )}%), ${authMetrics.publicEndpoints} public endpoint calls`
    );
  ...
  if (
    white_lists.find((item) => "/v1/api" + item === req.originalUrl) ||
